...
import asyncio, hashlib, logging, struct, time, os
...
class PeerServer:
    ...
    async def _handle_client(self, reader, writer):
        ...
        try:
            ...
            writer.write(our); await writer.drain()
            await self._send(writer, MessageID.BITFIELD, self._make_bitfield())
            await self._send(writer, MessageID.UNCHOKE)
            while self._running:
                try:
                    raw = await asyncio.wait_for(reader.readexactly(4), timeout=30.0)
                    length = struct.unpack(">I", raw)[0]
                    if length == 0: continue
                    body = await reader.readexactly(length)
                    msg_id = body[0]; payload = body[1:] if length > 1 else b""
    ...
    async def _on_extended(self, writer, payload):
        ...
        ext_id = payload[0]; ext_data = payload[1:]
        from .bencode import decode_all
        try: decoded = decode_all(ext_data)
        ...
        if ext_id == 0:
            ...
            await self._send(writer, EXTENDED_MESSAGE, bytes([0]) + encode(resp))
        else:
            ut_id = decoded.get(b"ut_metadata")
            if ut_id is not None and decoded.get(b"msg_type") == 0 and self.meta.info_data:
                piece = decoded.get(b"piece", 0)
                total = len(self.meta.info_data)
                bs = 16384; off = piece * bs
                chunk = self.meta.info_data[off:off+bs]
                resp = encode({b"msg_type": 1, b"piece": piece, b"total_size": total, b"data": chunk})
                await self._send(writer, EXTENDED_MESSAGE, bytes([1]) + resp)
    ...
    async def _on_request(self, writer, payload):
        ...
        try:
            ...
            return
        await self._send(writer, MessageID.PIECE, struct.pack(">II", index, begin) + data)

    async def _send(self, writer, msg_id, payload=b""):
        try:
            writer.write(struct.pack(">IB", 1 + len(payload), msg_id) + payload)
            await writer.drain()
        except Exception:
            pass
